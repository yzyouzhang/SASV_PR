...
def generate_spk_meta(config) -> None:
    d_spk_train = get_spkdic(config.cm_trn_list)
    d_spk_dev = get_spkdic(config.cm_dev_list)
    d_spk_eval = get_spkdic(config.cm_eval_list)
    os.makedirs(config.spk_meta, exist_ok=True)

    # save speaker dictionaries
    with open(config.spk_meta + "spk_meta_trn.pk", "wb") as f:
        pk.dump(d_spk_train, f)
    with open(config.spk_meta + "spk_meta_dev.pk", "wb") as f:
        pk.dump(d_spk_dev, f)
    with open(config.spk_meta + "spk_meta_eval.pk", "wb") as f:
